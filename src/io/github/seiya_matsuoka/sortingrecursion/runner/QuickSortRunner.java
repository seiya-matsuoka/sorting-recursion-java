...
import io.github.seiya_matsuoka.sortingrecursion.algorithms.QuickSortBasics;
import java.util.Arrays;

/** クイックソート実行用 runner */
public class QuickSortRunner implements TopicRunner {
  ...
  public void run(RunnerOptions options) {
    ...
    // 学習対象の本体処理を呼び出す部分
    QuickSortBasics.SortResult result = QuickSortBasics.sort(input, options.trace());

    System.out.println();
    System.out.println("ソート後配列: " + Arrays.toString(result.sortedArray()));
    System.out.println("比較回数: " + result.comparisonCount());
    System.out.println("交換回数: " + result.swapCount());
    System.out.println("参考実行時間(ns): " + result.elapsedNanos());

    if (!options.trace()) {
      System.out.println();
      System.out.println("trace を付けると、pivot と partition の流れを確認できる。");
    }
  }
   ...
   * 大きい入力を自動生成する。
   *
   * <p>逆順に近い入力にして、pivot の分かれ方を確認しやすくする。
   */
