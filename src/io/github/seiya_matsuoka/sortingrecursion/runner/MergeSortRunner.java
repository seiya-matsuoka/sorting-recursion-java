...
import io.github.seiya_matsuoka.sortingrecursion.algorithms.MergeSortBasics;
import java.util.Arrays;

/** マージソート実行用 runner */
public class MergeSortRunner implements TopicRunner {
  ...
  public void run(RunnerOptions options) {
    ...
    // 学習対象の本体処理を呼び出す部分
    MergeSortBasics.SortResult result = MergeSortBasics.sort(input, options.trace());

    System.out.println();
    System.out.println("ソート後配列: " + Arrays.toString(result.sortedArray()));
    System.out.println("比較回数: " + result.comparisonCount());
    System.out.println("配列への書き込み回数: " + result.writeCount());
    System.out.println("参考実行時間(ns): " + result.elapsedNanos());

    if (!options.trace()) {
      System.out.println();
      System.out.println("trace を付けると、分割とマージの流れを確認できる。");
    }
  }
   ...
   * 大きい入力を自動生成する。
   *
   * <p>逆順に近い入力にして、分割後のマージ結果を追いやすくする。
   */
